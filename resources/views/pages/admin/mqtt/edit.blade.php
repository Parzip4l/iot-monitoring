...
@section('content')
<x-breadcrub pagetitle="LRTJ" subtitle="MQTT Management" title="Edit Setting" />

<div class="container-fluid">
    <div class="page-content-wrapper">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Edit Setting MQTT</h5>
            ...
            <div class="card-body">
                ...
                <!-- Form Edit Setting -->
                <form action="{{ route('mqtt.update', $setting->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="topic" id="topic"
                            ...
                            value="{{ old('topic', $setting->topic) }}" required>
                        @error('topic')
                    ...
                    <div class="mb-3">
                        ...
                        <select name="device_id" id="device_id"
                            ...
                            @foreach($device as $d)
                                <option value="{{ $d->id }}" 
                                    {{ old('device_id', $setting->device_id) == $d->id ? 'selected' : '' }}>
                                    {{ $d->name ?? $d->device_id }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="interval" id="interval"
                            ...
                            value="{{ old('interval', $setting->interval) }}" min="5" required>
                        @error('interval')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
